...
void GetSinWave(int point, int max_amp, int amp_val, u16 *sin_tab)
{
	...
	{
		x = radian*i ;
		sin_tab[i] = float2fixed((amp_val/2)*sin(x) + max_amp/2) ;
	}
}
...
void GetSquareWave(int point, int max_amp, int amp_val, u16 *Square_tab)
{
	...
	{
		if (i < point/2)
			Square_tab[i] = float2fixed((max_amp-amp_val)/2 + amp_val - 1) ;
		else
			Square_tab[i] = float2fixed((max_amp-amp_val)/2 ) ;
	}
}
...
void GetTriangleWave(int point, int max_amp, int amp_val, u16 *Triangle_tab)
{
	...
	{
		if (i < point/2)
			Triangle_tab[i] = float2fixed(i*tap_val + (max_amp-amp_val)/2) ;
		else
			Triangle_tab[i] = float2fixed(amp_val - 1 - (i-point/2)*tap_val + (max_amp-amp_val)/2 ) ;
	}
}
...
void GetSawtoothWave(int point, int max_amp, int amp_val, u16 *Sawtooth_tab)
{
	...
	for(i = 0; i < point; i++)
		Sawtooth_tab[i] = float2fixed(i*tap_val + (max_amp-amp_val)/2) ;
}
...
void GetSubSawtoothWave(int point, int max_amp, int amp_val, u16 *SubSawtooth_tab)
{
	...
	for(i = 0; i < point; i++)
		SubSawtooth_tab[i] = float2fixed(amp_val - 1 - i*tap_val + (max_amp-amp_val)/2) ;
}

u16 float2fixed(double value) {
	return (u16)(value * (1 << 7));
}
